...
## -- Set color palette for DSD, infants, preschoolers -------------------------
## One chart will exclude preschoolers, so need a way to have consistency vs
## relying on defaults
# age_palette <- viridisLite::viridis(3) ## journal charges for color
age_palette <- c("black", "gray20", "gray40")
age_shapes <- c(15, 17, 19)
...
#                                as.character(radar.data$symptom))
  ...
  dot.plot <-
  ggplot(aes(x = usex, y = pct, colour = pop, shape = pop), data = radar.data) +
    geom_vline(xintercept = 1:length(del.symptoms), colour = 'grey90', alpha = 0.7, size = 20) +
    geom_hline(yintercept = seq(25, 100, 25), colour = 'white', size = 0.5) +
    geom_vline(xintercept = c(1:length(del.symptoms)), colour = 'grey99', size = 0.1) +
    geom_beeswarm(size = 3, alpha = 0.7, priority = 'ascending') +
    scale_color_manual(name = "", values = age_palette) +
    scale_shape_manual(name = "", values = age_shapes) +
