...
pediatric vs the geriatric populations. For each population separately, we cut a hierarchical
cluster tree; to avoid issues of correlation within patients, we took only the first day from each
patient. Once we determined clusters for each population, we used a Cohen's kappa to assess
``agreement" between symptom clusters of the two populations.
