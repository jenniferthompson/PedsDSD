...
\begin{landscape}
<<symptomsallfig, fig.cap='Dot Chart, Delirium Symptoms among All Assessments by Population', fig.height=8, fig.width=11>>=
# radar.all$dot.plot
...
To see if the pediatric and geriatric populations had similar clusters of symptoms when assessed for
delirium/dementia, we performed a cluster analysis. For each population separately, we cut a
hierarchical cluster tree, grouping symptoms by similarity. (To avoid issues of correlation within
patients, we took only the first day from each patient.) Once we determined clusters for each
population, we used a Cohen's kappa to assess ``agreement" between symptom clusters of the two
populations.

<<cluster_datamgmt, results = 'asis', fig.height = 5, fig.width = 8>>=
library(ClustOfVar) ## cluster analysis
...
dsd.tree <- hclustvar(X.quanti = dsd.clusterdata)
## Columns need to be numeric; make pscam.clusterdata a numeric matrix
pscam.tree <- hclustvar(X.quanti = as.matrix(pscam.clusterdata)*1)
